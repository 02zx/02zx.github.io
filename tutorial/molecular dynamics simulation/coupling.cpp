...
{
    real p_corr_z = 0;
    real pressure_amplitude = 0;
    real coupling_period = 0;
    switch (pressureCouplingOptions.epct)
    {
        case PressureCouplingType::Isotropic:
            for (int d = 0; d < DIM; d++)
            ...
            break;
        case PressureCouplingType::SemiIsotropic:
            coupling_period = pressureCouplingOptions.compress[ZZ][ZZ];
            pressure_amplitude = pressureCouplingOptions.ref_p[ZZ][ZZ];
            for (int d = 0; d < DIM; d++)
            {
                (*mu)(d, d) = 1.0
                              - compressibilityFactor(0, 0, pressureCouplingOptions, couplingTimePeriod)
                                        * (pressureCouplingOptions.ref_p[0][0] - scalar_pressure + pressure_amplitude*sin(2*3.1416*step/coupling_period)) / DIM;
           // + pressure_amplitude*sin(2*Pi*t/coupling_period) for ultrasound;pressure_amplitude = xx bar, coupling_period=xxx step.
           // ------------------------------------------------------------original code-------------------------------------------------------------------------------------
           // for (int d = 0; d < ZZ; d++)
           // {
           //     (*mu)(d, d) = 1.0
